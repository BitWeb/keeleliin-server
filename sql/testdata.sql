...
INSERT INTO service (id, name, url) SELECT 1, 'test service', 'http://dev.bitweb.ee/api/v1/' WHERE NOT EXISTS (SELECT 1 FROM service WHERE service.id = 1);
...
INSERT INTO workflow_definition_service (id, workflow_definition_id, service_id, order_num, created_at, updated_at)
SELECT 2, 1, 1, 1, NOW(), NOW() WHERE NOT EXISTS (SELECT 1 FROM workflow_definition_service WHERE workflow_definition_service.id = 2);
...
INSERT INTO service_input_type(
            id, key, do_parallel, service_id, resource_type_id)
    VALUES (1, 'content', FALSE, 1, 1);
